...
window.onload = function () {
  ...
  if (nombreJugador) {
    document.getElementById("nombreDelJugador").innerText = nombreJugador;
  } else {
    document.getElementById("nombreDelJugador").innerText = "Jugador Anónimo";
  }
};

document.getElementById("toggleSound").addEventListener("click", function () {
  soundEnabled = !soundEnabled; // invertir el valor de soundEnabled

  if (soundEnabled) {
    this.innerText = "Desactivar Sonido";
    sonido_juegonuevo.play(); // reproducir el sonido aquí
  } else {
    this.innerText = "Activar Sonido";
  }
});
if (!soundEnabled) {
  sonido_juegonuevo.pause();
  sonido_juegonuevo.currentTime = 0; // esto devuelve el audio al inicio
}
...
function mostrarCreditos() {
  Swal.fire({
    ...
    html: `
          ...
          <!-- Puedes continuar agregando más créditos aquí -->
      `,
...
document
  .getElementById("nombreDelJugador")
